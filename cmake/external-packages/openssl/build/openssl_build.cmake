...
ProcessorCount(PLATFORM_PROCESSOR_COUNT)

if(WIN32 AND NOT CYGWIN)
  option(USE_JOM_NMAKE "Force use of JOM to build OpenSSL (Experimental)" OFF)
  if(NOT USE_JOM_NMAKE AND DEFINED ENV{USE_JOM_NMAKE})
    set(USE_JOM_NMAKE "$ENV{USE_JOM_NMAKE}")
  endif()
endif()
...
macro(openssl_build_setup_unix out_setup_rules platform)
  ...
  unset(MAKE_ARGS)
  multi_list(APPEND ${out_setup_rules} EXEC make ${MAKE_ARGS} all)
...
macro(openssl_build_setup_windows out_setup_rules)
  # Check 'nasm' tool is accessible from scripts
  find_program(NASM_PROGRAM nasm)
  if(NOT NASM_PROGRAM)
    external_error("No 'nasm' tool found")
  endif()

  unset(MAKE_PROGRAM)
  set(MAKE_ARGS /nologo)
  if(USE_JOM_NMAKE)
    find_program(MAKE_PROGRAM jom)
    if(PLATFORM_PROCESSOR_COUNT)
      list(APPEND MAKE_ARGS /J ${PLATFORM_PROCESSOR_COUNT})
    endif()
  ...
  if(NOT MAKE_PROGRAM)
    external_error("No 'nmake' tool found")
  endif()
  ...
  if(OPENSSL_BUILD_STATIC)
    multi_list(APPEND ${out_setup_rules}
      WRAP ${MAKE_PROGRAM}
      ARGS ${MAKE_ARGS} /f "${OPENSSL_SOURCE_DIR}/ms/nt.mak"
    ...
    multi_list(APPEND ${out_setup_rules}
      WRAP ${MAKE_PROGRAM}
      ARGS ${MAKE_ARGS} /f "${OPENSSL_SOURCE_DIR}/ms/ntdll.mak"
