...
public:
  ...
  /// Type of the object identifying a fiber (with a remote and a local fiber port)
  typedef typename Service::fiber_id fiber_id;

private:
  typedef boost::asio::fiber::detail::basic_pending_accept_operation<StreamSocket> accept_op;
...
public:
  ...
  */
                ...
                size_t mtu = 60 * 1024)
  {
    if (mtu > 60 * 1024)
    {
      BOOST_LOG_TRIVIAL(warning) << "fiber demux: MTU too big, replaced with MAX_VALUE";
      mtu = 60 * 1024;
    }
  ...
  *
  * @param local_port A local fiber port to which the fiber acceptor will be bound.
  *
  ...
  * @param ec Set to indicate what error occurred, if any.
  */
  void listen(local_port_type local_port,
              boost::system::error_code& ec)
  {
  ...
  *
  * @param remote_port The remote fiber port to which the fiber should be connected.
  *
  * @param fib_impl The implementation object of the fiber.
  *
  * @note When the connection process is finished, the handler connect_user_handler
  * of the implementation object will be called.
  */
  void async_connect(remote_port_type remote_port,
  ...
  */
  ...
  void async_send_dgr(const ConstBufferSequence& buffers,
                      ...
                      Handler& handler)
  {
    return service_.async_send_datagram(impl_,
                                        remote_port,
                                        fib_impl,
                                        buffers,
                                        handler);
  ...
  /// Close fiber.
  /**
  * This closes a fiber immediatly. It cancels all pending operations from this fiber.
  */
