...
#ifndef SSF_COMMON_BOOST_ASIO_FIBER_DETAIL_IO_FIBER_DGR_READ_OP_HPP_
#define SSF_COMMON_BOOST_ASIO_FIBER_DETAIL_IO_FIBER_DGR_READ_OP_HPP_
...
/// Class to store buffers and read datagram operations on a fiber
/**
...
class pending_dgr_read_operation : public basic_pending_dgr_read_operation
{
 public:
  BOOST_ASIO_DEFINE_HANDLER_PTR(basic_pending_dgr_read_operation);
  ...
  */
  pending_dgr_read_operation(const MutableBufferSequence& buffers, Handler& handler)
    : basic_pending_dgr_read_operation(&pending_dgr_read_operation::do_complete,
                                   &pending_dgr_read_operation::do_fill_buffers,
                                   nullptr),
    buffers_(buffers),
    handler_(handler)
  {
  ...
  * received
  */
  pending_dgr_read_operation(const MutableBufferSequence& buffers,
                         Handler& handler,
                         fiber_id::remote_port_type* p_remote_port)
                         : basic_pending_dgr_read_operation(
                             &pending_dgr_read_operation::do_complete,
                             &pending_dgr_read_operation::do_fill_buffers,
                             p_remote_port),
                           buffers_(buffers),
                           handler_(handler)
  ...
  */
                          ...
                          std::size_t bytes_transferred)
  {
    ...
    pending_dgr_read_operation* o(static_cast<pending_dgr_read_operation*>(base));
  ...
  */
  ...
  {
    pending_dgr_read_operation* o(static_cast<pending_dgr_read_operation*>(base));
    return boost::asio::buffer_copy(o->buffers_, boost::asio::buffer(data));
  }
...
#endif  // SSF_COMMON_BOOST_ASIO_FIBER_DETAIL_IO_FIBER_DGR_READ_OP_HPP_
