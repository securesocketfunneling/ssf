...
#include <boost/chrono/duration.hpp>  // NOLINT
...
struct DatagramLink : public std::enable_shared_from_this<
 ...
 public:
  ...
  void Feed(boost::asio::const_buffer buffer, size_t length) {
    boost::recursive_mutex::scoped_lock lock(datagram_queue_mutex_);
    ...
    reenter(coro_external_) {
      for (;;) {
        ...
        if (!ec && dgr_queue_.empty()) {
          ...
          yield timer_.async_wait(boost::bind(&DatagramLink::ExternalFeed,
                                              this->shared_from_this(), _1, 0));
        }
    ...
    if (!ec && r_.is_open() && l_.is_open()) reenter(coro_auto_) {
        for (;;) {
          yield l_.async_receive_from(
              ...
              boost::bind(&DatagramLink::AutoFeed, this->shared_from_this(), _1,
                          _2));
          ...
          while (transferred_bytes_ < bytes_to_transfer_) {
            yield r_.async_send_to(
                ...
                boost::bind(&DatagramLink::AutoFeed, this->shared_from_this(),
                            _1, _2));
