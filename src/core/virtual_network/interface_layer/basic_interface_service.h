#ifndef SSF_CORE_VIRTUAL_NETWORK_INTERFACE_LAYER_BASIC_INTERFACE_SERVICE_H_
#define SSF_CORE_VIRTUAL_NETWORK_INTERFACE_LAYER_BASIC_INTERFACE_SERVICE_H_
...
#include "core/virtual_network/interface_layer/specific_interface_socket.h"
...
class basic_Interface_service
 ...
 public:
  ...
  void destroy(implementation_type& impl) {
    {
      boost::recursive_mutex::scoped_lock lock(
          protocol_type::get_interface_manager().mutex);
      protocol_type::get_interface_manager().available_sockets.erase(
          impl.interface_name);
    }
  ...
  BOOST_ASIO_INITFN_RESULT_TYPE(ConnectHandler, void(boost::system::error_code))
    ...
    impl.interface_name = std::move(interface_name);
    impl.p_next_socket =
        std::make_shared<typename raw_next_layer_protocol::socket>(
            this->get_io_service());

    auto connected_lambda = [this, init, &impl](
      ...
      if (!ec) {
        ...
        auto p_specific_socket = std::make_shared<
            specific_interface_socket<Protocol, NextLayerProtocol>>(
            impl.p_next_socket);

        auto endpoint_inserted =
            protocol_type::get_interface_manager().available_sockets.emplace(
  ...
  BOOST_ASIO_INITFN_RESULT_TYPE(AcceptHandler, void(boost::system::error_code))
    ...
    impl.p_next_acceptor->set_option(boost::asio::socket_base::reuse_address(true), ec);
...
#endif  // SSF_CORE_VIRTUAL_NETWORK_INTERFACE_LAYER_BASIC_INTERFACE_SERVICE_H_
