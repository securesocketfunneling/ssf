#ifndef SSF_CORE_VIRTUAL_NETWORK_INTERFACE_LAYER_BASIC_INTERFACE_H_
#define SSF_CORE_VIRTUAL_NETWORK_INTERFACE_LAYER_BASIC_INTERFACE_H_
...
class basic_Interface : public boost::asio::basic_io_object<InterfaceService> {
 private:
  ...
  typedef typename InterfaceService::endpoint_context_type
      endpoint_context_type;
 ...
 public:
  ...
  BOOST_ASIO_INITFN_RESULT_TYPE(ConnectHandler, void(boost::system::error_code))
      async_connect(endpoint_context_type interface_name,
                    const typename raw_next_layer_protocol::
                        endpoint& next_remote_endpoint,
  ...
  bool is_open() const { return this->get_service().is_open(this->implementation); }
};

}  // interface_layer
}  // virtual_network

#endif  // SSF_CORE_VIRTUAL_NETWORK_INTERFACE_LAYER_BASIC_INTERFACE_H_
