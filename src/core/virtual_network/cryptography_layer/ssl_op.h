 ...
 void operator()(
   ...
   if (!ec) {
     reenter(coro_) {
       ...
       *p_local_endpoint_ =
           Endpoint(stream_.next_layer().local_endpoint(endpoint_ec));
...
class SSLAcceptOp {
public:
 SSLAcceptOp(Acceptor& acceptor, typename std::remove_reference<PeerImpl>::type& peer_impl,
    ...
    if (!ec) {
      reenter(coro_) {
        yield acceptor_.async_accept(
            peer_impl_.p_next_layer_socket->next_layer(),
            peer_impl_.p_remote_endpoint->next_layer_endpoint()
                .next_layer_endpoint(),
            std::move(*this));

        peer_impl_.p_remote_endpoint->next_layer_endpoint().set();
        ...
        peer_impl_.p_next_layer_socket->async_handshake(
