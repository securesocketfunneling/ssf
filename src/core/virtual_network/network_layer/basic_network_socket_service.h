#ifndef SSF_CORE_VIRTUAL_NETWORK_NETWORK_LAYER_BASIC_NETWORK_SOCKET_SERVICE_H_
#define SSF_CORE_VIRTUAL_NETWORK_NETWORK_LAYER_BASIC_NETWORK_SOCKET_SERVICE_H_
#include <memory>
  ...
  BOOST_ASIO_INITFN_RESULT_TYPE(ConnectHandler, void(boost::system::error_code))
    ...
    this->get_io_service().post(
        io::ComposedOp<decltype(connect_lambda), ConnectHandler>(
            [this, &impl, remote_endpoint, init]() {
              boost::system::error_code ec;
 ...
 private:
  ...
  void register_async_op() {
    ...
    if (usage_count_ == 0) {
      ...
      // std::make_unique<boost::asio::io_service::work>(
      //           this->get_io_service());
...
#endif  // SSF_CORE_VIRTUAL_NETWORK_NETWORK_LAYER_BASIC_NETWORK_SOCKET_SERVICE_H_
