...
void SystemInterfaces::Start(uint16_t nb_threads, int remount_delay) {
  if (!p_worker_) {
    p_worker_ = std::unique_ptr<boost::asio::io_service::work>(
      new boost::asio::io_service::work(io_service_));
    p_threads_ = std::unique_ptr<boost::thread_group>(
      new boost::thread_group());
    for (uint16_t i = 1; i <= nb_threads; ++i) {
      auto& io_service = io_service_;
      p_threads_->create_thread([&io_service]() { io_service.run(); });
    }
