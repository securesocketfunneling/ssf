...
void SocksServer<Demux>::start(boost::system::error_code& ec) {
  BOOST_LOG_TRIVIAL(info) << "SOCKS: starting server on port " << local_port_;
  ec = init_ec_;
...
void SocksServer<Demux>::stop(boost::system::error_code& ec) {
  ...
  BOOST_LOG_TRIVIAL(info) << "SOCKS: stopping server";
  this->HandleStop();
}
...
void SocksServer<Demux>::HandleAccept(const boost::system::error_code& ec) {
  BOOST_LOG_TRIVIAL(trace) << "SOCKS: HandleAccept";
  ...
  auto start_handler =
      [this, self, p_version](boost::system::error_code ec, std::size_t) {
        if (ec) {
          BOOST_LOG_TRIVIAL(error)
              << "SOCKS: error reading protocol version: " << ec.message()
              << " " << ec.value();
          fiber fib = std::move(this->new_connection_);
          this->StartAccept();
        } else if (p_version->Number() == 4) {
          BOOST_LOG_TRIVIAL(trace) << "SOCKS: version accepted: v4";
          ...
          BOOST_LOG_TRIVIAL(trace) << "SOCKS: version accepted: v5";
          ssf::BaseSessionPtr new_socks_session =
          ...
          BOOST_LOG_TRIVIAL(error)
              << "SOCKS: protocol not supported yet: " << p_version->Number();
