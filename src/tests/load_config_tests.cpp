...
TEST_F(LoadConfigTest, loadPartialFile) {
  ...
  ASSERT_EQ(config.tls.key_password, "");
  ASSERT_EQ(config.tls.dh_path, "test_dh_path");
  ASSERT_EQ(config.tls.cipher_alg, "DHE-RSA-AES256-GCM-SHA384");
}
...
TEST_F(LoadConfigTest, loadCompleteFile) {
  ...
  ASSERT_EQ(config.tls.key_path, "test_key_path");
  ASSERT_EQ(config.tls.key_password, "test_key_password");
  ASSERT_EQ(config.tls.dh_path, "test_dh_path");
  ASSERT_EQ(config.tls.cipher_alg, "test_cipher_alg");
}
